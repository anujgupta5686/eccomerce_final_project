...
const Header = () => {
  ...
  console.log("Header User::", user?.role);
        ...
        {user?.role !== ROLE.ADMIN && (
          <div className="hidden lg:flex items-center w-full justify-between max-w-sm border rounded-full focus-within:shadow pl-2">
            <input
              ...
              value={search}
            />
            <div className="text-lg min-w-[50px] h-8 bg-cyan-500 flex items-center justify-center rounded-r-full text-white">
              <GrSearch />
            </div>
          </div>
        )}
        ...
        <div className="flex items-center gap-7">
          ...
          {user?._id && user?.role !== ROLE.ADMIN && (
            <Link to={"/cart"} className="text-2xl relative">
              ...
              <div className="bg-cyan-500 text-white w-5 h-5 rounded-full p-1 flex items-center justify-center absolute -top-2 -right-3">
          ...
          <div>
            {user?._id ? (
              <button
                ...
                className="px-3 py-1 rounded-full text-white bg-cyan-500 hover:bg-cyan-600"
              >
                Logout
              </button>
            ) : (
